...
initClientErrors({
...
});`,Ke=async()=>new Promise(e=>{globalThis.requestAnimationFrame(()=>{globalThis.requestAnimationFrame(()=>e())})}),Pr=()=>{throw new Error("Division by zero in checkout pricing engine")},te=(e,r)=>{var n;if(!r){e.textContent="No payload received yet.";return}let t=(n=r.screenshot)!=null&&n.dataUrl?{...r,screenshot:{...r.screenshot,dataUrl:`${r.screenshot.dataUrl.slice(0,180)}... [truncated in docs preview]`}}:r;e.textContent=JSON.stringify(t,null,2)},J=(e,r,t,n)=>{var a,s,i,l;let o=(a=n==null?void 0:n.screenshot)==null?void 0:a.dataUrl;if(!o){e.hidden=!0,e.removeAttribute("src"),r.hidden=!1,t.textContent="The next captured error will render a full-screen screenshot of the page here.";return}e.src=o,e.alt="Captured screenshot of the client-errors playground action",e.hidden=!1,r.hidden=!0,t.textContent=`${(i=(s=n==null?void 0:n.screenshot)==null?void 0:s.format)!=null?i:"image/png"} attached to event ${(l=n==null?void 0:n.eventId)!=null?l:"unknown"}`},Fe=e=>{var oe,se,ie,ae,de,le,ce,ue;let r=console.error.bind(console),t=typeof window!="undefined"&&window.location.hostname.endsWith("github.io"),n=[];e.innerHTML=`
      ...
      <div class="playground__grid">
        ...
        <div class="playground__preview">
          <div class="preview-card">
            ...
            ${t?'<div class="status-badge">Static preview mode: no network requests</div>':""}
  ...
  `;let o=e.querySelector("#playground-endpoint"),a=e.querySelector("#playground-auth"),s=e.querySelector("#playground-console-errors"),i=e.querySelector("#playground-code"),l=e.querySelector("#playground-output"),d=e.querySelector("#playground-status"),c=e.querySelector("#playground-count"),u=e.querySelector("#playground-screenshot-image"),m=e.querySelector("#playground-screenshot-empty"),f=e.querySelector("#playground-screenshot-meta"),C=e.querySelector("#playground-demo-surface"),E=e.querySelector("#playground-demo-phase"),b=e.querySelector("#playground-demo-action"),g=e.querySelector("#playground-demo-note"),w=e.querySelector("#playground-demo-error"),R=e.querySelector("#playground-demo-click"),P=e.querySelector("#playground-demo-total"),q=e.querySelector("#playground-demo-divider");if(!o||!a||!s||!i||!l||!d||!c||!u||!m||!f||!C||!E||!b||!g||!w||!R||!P||!q)return;let x=()=>{C.dataset.phase="ready",E.textContent="Checkout ready",b.textContent="Waiting for a user action",g.textContent="Click the button to simulate a broken checkout rule that divides by zero.",R.textContent="No click recorded yet",P.textContent="Pending calculation",q.textContent="0",w.hidden=!0,w.textContent=""},h=()=>{i.textContent=Tr(o.value,a.value,s.checked)},D=async()=>{var y,L,_,H,pe;if(t){let G=(y=n[n.length-1])!=null?y:null;c.textContent=`Prepared events: ${n.length}`,te(l,G),J(u,m,f,G);return}try{let X=await(await fetch("api/frontend-errors/logs")).json(),me=(H=(_=(L=X.items)==null?void 0:L[X.items.length-1])==null?void 0:_.payload)!=null?H:null;c.textContent=`Server events: ${(pe=X.count)!=null?pe:0}`,te(l,me),J(u,m,f,me)}catch{c.textContent="Server events: unavailable",l.textContent="Unable to read server logs.",J(u,m,f,null)}},ne=async()=>{We(),n.splice(0,n.length);let y=o.value.trim(),L=a.value,_=s.checked;$e({endpoint:y,appName:"docs-playground",environment:"development",release:"0.1.0",captureWindowErrors:!0,captureUnhandledRejections:!0,captureConsoleErrors:_,screenshot:{enabled:!0,format:"image/png",includeInPayload:!0,maxWidth:1440,maxHeight:1200},auth:L==="bearer"?{type:"bearer",token:"public-ingest-token"}:L==="custom"?{type:"custom",headers:{"X-Ingest-Key":"demo-public-key"}}:{type:"none"},beforeSend:H=>t?(n.push(H),d.textContent="Payload prepared locally",D(),null):H,onSuccess:()=>{d.textContent="Payload delivered",D()},onError:()=>{d.textContent="Transport or SDK step failed silently"}}),d.textContent="SDK initialized",h(),t||await fetch("api/frontend-errors/logs",{method:"DELETE"}),await D()};(oe=e.querySelector("#playground-init"))==null||oe.addEventListener("click",()=>{ne()}),(se=e.querySelector("#playground-clear"))==null||se.addEventListener("click",async()=>{n.splice(0,n.length),t||await fetch("api/frontend-errors/logs",{method:"DELETE"}),te(l,null),J(u,m,f,null),c.textContent=t?"Prepared events: 0":"Server events: 0",d.textContent=t?"Prepared payloads cleared":"Server logs cleared",x()}),(ie=e.querySelector("#playground-capture-message"))==null||ie.addEventListener("click",async()=>{V({type:"manual",value:"captureMessage button clicked"}),await Ne("Playground informational message","warn",{custom:{channel:"docs-playground"}}),await Q()}),(ae=e.querySelector("#playground-trigger-console"))==null||ae.addEventListener("click",async()=>{console.error("Playground console.error sample"),await Q()}),(de=e.querySelector("#playground-trigger-rejection"))==null||de.addEventListener("click",()=>{Promise.reject(new Error("Playground unhandled rejection"))}),(le=e.querySelector("#playground-breadcrumb"))==null||le.addEventListener("click",()=>{V({type:"manual",value:"Manual breadcrumb from playground"}),d.textContent="Breadcrumb added"}),(ce=e.querySelector("#playground-reset-demo"))==null||ce.addEventListener("click",()=>{x(),d.textContent="Demo surface reset"}),(ue=e.querySelector("#playground-trigger-demo"))==null||ue.addEventListener("click",async()=>{V({type:"manual",value:"Checkout CTA clicked in playground"}),C.dataset.phase="processing",E.textContent="Repricing order",b.textContent="Checkout CTA clicked",g.textContent="The UI is recalculating the order total before the failure is reported.",R.textContent="Clicked: Trigger checkout error",P.textContent="Recomputing...",w.hidden=!0,w.textContent="",d.textContent="Simulating checkout failure",await Ke();try{let y=Pr();P.textContent=`$${y.toFixed(2)}`,g.textContent="Calculation unexpectedly succeeded."}catch(y){let L=y instanceof Error?y.message:"Unknown checkout runtime error";C.dataset.phase="failed",E.textContent="Checkout failed",b.textContent="Division error visible in the UI",g.textContent="This visible error state is rendered first so the screenshot shows the exact broken page state.",P.textContent="Order total unavailable",w.hidden=!1,w.textContent=L,d.textContent="Division error captured, flushing queue",r("Playground checkout error:",y),await Ke(),await Oe(y,{source:"docs.checkout-demo",custom:{demo:"checkout-surface",action:"trigger-checkout-error",divider:0,orderId:"demo-order-2048",surfaceState:"failed"}}),await Q()}}),o.addEventListener("input",h),a.addEventListener("change",h),s.addEventListener("change",h),h(),x(),D(),ne()};var Qe=document.querySelector("#app");if(!Qe)throw new TypeError("Docs app root was not found.");var Lr=Z.map(e=>`
