...
`,Y=[{id:"overview",title:"Overview",eyebrow:"Browser SDK",description:"Capture browser runtime errors and send a normalized JSON payload to an endpoint you control.",body:`
    ...
    `},{id:"installation",title:"Installation",eyebrow:"Quick start",description:"Install the package and initialize it with your ingest endpoint.",body:`
      ${L("npm install @revivejs/client-errors","bash")}
      ${L(`import { initClientErrors } from "@revivejs/client-errors";
...
initClientErrors({
  ...
  `;let o=e.querySelector("#playground-endpoint"),a=e.querySelector("#playground-auth"),i=e.querySelector("#playground-console-errors"),s=e.querySelector("#playground-code"),d=e.querySelector("#playground-output"),l=e.querySelector("#playground-status"),c=e.querySelector("#playground-count"),u=e.querySelector("#playground-screenshot-image"),m=e.querySelector("#playground-screenshot-empty"),C=e.querySelector("#playground-screenshot-meta"),v=e.querySelector("#playground-demo-surface"),S=e.querySelector("#playground-demo-phase"),x=e.querySelector("#playground-demo-action"),b=e.querySelector("#playground-demo-note"),f=e.querySelector("#playground-demo-error"),g=e.querySelector("#playground-demo-click"),k=e.querySelector("#playground-demo-total"),H=e.querySelector("#playground-demo-divider");if(!o||!a||!i||!s||!d||!l||!c||!u||!m||!C||!v||!S||!x||!b||!f||!g||!k||!H)return;let z=()=>{v.dataset.phase="ready",S.textContent="Checkout ready",x.textContent="Waiting for a user action",b.textContent="Click the button to simulate a checkout rule that divides by zero.",g.textContent="No click recorded yet",k.textContent="Pending calculation",H.textContent="0",f.hidden=!0,f.textContent=""},P=()=>{s.textContent=$r(o.value,a.value,i.checked)},I=async()=>{var A,E,U,_,me;if(t){let J=(A=n[n.length-1])!=null?A:null;c.textContent=`Prepared events: ${n.length}`,ne(d,J),X(u,m,C,J);return}try{let Z=await(await fetch("api/frontend-errors/logs")).json(),fe=(_=(U=(E=Z.items)==null?void 0:E[Z.items.length-1])==null?void 0:U.payload)!=null?_:null;c.textContent=`Server events: ${(me=Z.count)!=null?me:0}`,ne(d,fe),X(u,m,C,fe)}catch{c.textContent="Server events: unavailable",d.textContent="Unable to read server logs.",X(u,m,C,null)}},oe=async()=>{Je(),n.splice(0,n.length);let A=o.value.trim(),E=a.value,U=i.checked;Qe({endpoint:A,appName:"docs-playground",environment:"development",release:"0.1.1",captureWindowErrors:!0,captureUnhandledRejections:!0,captureConsoleErrors:U,dom:{enabled:!0},sourceContext:{enabled:!0,contextLines:2},screenshot:{enabled:!0,format:"image/png",includeInPayload:!0,maxWidth:1440,maxHeight:1200},auth:E==="bearer"?{type:"bearer",token:"public-ingest-token"}:E==="custom"?{type:"custom",headers:{"X-Ingest-Key":"demo-public-key"}}:{type:"none"},beforeSend:_=>t?(n.push(_),l.textContent="Payload prepared locally",I(),null):_,onSuccess:()=>{l.textContent="Payload delivered",I()},onError:()=>{l.textContent="Transport or SDK step failed silently"}}),l.textContent="SDK initialized",P(),t||await fetch("api/frontend-errors/logs",{method:"DELETE"}),await I()};(ie=e.querySelector("#playground-init"))==null||ie.addEventListener("click",()=>{oe()}),(se=e.querySelector("#playground-clear"))==null||se.addEventListener("click",async()=>{n.splice(0,n.length),t||await fetch("api/frontend-errors/logs",{method:"DELETE"}),ne(d,null),X(u,m,C,null),c.textContent=t?"Prepared events: 0":"Server events: 0",l.textContent=t?"Prepared payloads cleared":"Server logs cleared",z()}),(ae=e.querySelector("#playground-capture-message"))==null||ae.addEventListener("click",async()=>{Q({type:"manual",value:"captureMessage button clicked"}),await Xe("Playground informational message","warn",{custom:{channel:"docs-playground"}}),await G()}),(le=e.querySelector("#playground-trigger-console"))==null||le.addEventListener("click",async()=>{console.error("Playground console.error sample"),await G()}),(de=e.querySelector("#playground-trigger-rejection"))==null||de.addEventListener("click",()=>{Promise.reject(new Error("Playground unhandled rejection"))}),(ce=e.querySelector("#playground-breadcrumb"))==null||ce.addEventListener("click",()=>{Q({type:"manual",value:"Manual breadcrumb from playground"}),l.textContent="Breadcrumb added"}),(ue=e.querySelector("#playground-reset-demo"))==null||ue.addEventListener("click",()=>{z(),l.textContent="Demo surface reset"}),(pe=e.querySelector("#playground-trigger-demo"))==null||pe.addEventListener("click",async A=>{Q({type:"manual",value:"Checkout CTA clicked in playground"}),v.dataset.phase="processing",S.textContent="Repricing order",x.textContent="Checkout CTA clicked",b.textContent="The UI is recalculating the order total before the failure is reported.",g.textContent="Clicked: Trigger checkout error",k.textContent="Recomputing...",f.hidden=!0,f.textContent="",l.textContent="Simulating checkout failure",await Ze();try{let E=Or();k.textContent=`$${E.toFixed(2)}`,b.textContent="Calculation unexpectedly succeeded."}catch(E){let U=E instanceof Error?E.message:"Unknown checkout runtime error";v.dataset.phase="failed",S.textContent="Checkout failed",x.textContent="Division error visible in the UI",b.textContent="This visible error state is rendered first so the screenshot shows the exact broken page state.",k.textContent="Order total unavailable",f.hidden=!1,f.textContent=U,l.textContent="Division error captured, flushing queue",r("Playground checkout error:",E),await Ze(),await Ge(E,{source:"docs.checkout-demo",target:A.currentTarget,custom:{demo:"checkout-surface",action:"trigger-checkout-error",divider:0,orderId:"demo-order-2048",surfaceState:"failed"}}),await G()}}),o.addEventListener("input",P),a.addEventListener("change",P),i.addEventListener("change",P),P(),z(),I(),oe()};var rr=document.querySelector("#app");if(!rr)throw new TypeError("Docs app root was not found.");var Nr=Y.map(e=>`
